...
public class TaskInfoFragment extends Fragment implements View.OnClickListener{
    ...
    public void displayTask(TaskListContent.Task task)
    {
        ...
        taskInfoModel.setText ( task.model );
        taskInfoDescription.setText ( "Kolor: "+ task.color + "\n\nOpis: " + task.details);
        if(task.picPath!=null && !task.picPath.isEmpty ())
        {
            if(task.picPath.contains ( "drawable" ))
            {
                ...
                {
                    case "drawable 1":
                        taskDrawable = activity.getResources ().getDrawable ( R.drawable.zuk );
                        break;
                    case "drawable 2":
                        taskDrawable = activity.getResources ().getDrawable ( R.drawable.kadett );
                        break;
                    case "drawable 3":
                        taskDrawable = activity.getResources ().getDrawable ( R.drawable.kamaz );
                        break;
                    default:
                        taskDrawable = activity.getResources ().getDrawable ( R.drawable.zuk );
                }
            ...
            else
            {
                ...
                handler.postDelayed ( new Runnable () {
                    ...
                    public void run() {
                        ...
                        Bitmap cameraImage = PicUtils.decodePic ( mDiplayedTask.picPath, taskInfoImage.getWidth (), taskInfoImage.getHeight () );
        ...
        {
            taskInfoImage.setImageDrawable ( activity.getResources ().getDrawable ( R.drawable.zuk ) );
        }
        mDiplayedTask = task;
    }
    ...
    private File createImageFile() throws IOException{
        ...
        String imageFileName = mDiplayedTask.model + timeStamp + "_";
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data)
    {
        if(requestCode == REQUEST_IMAGE_CAPTURE && resultCode == Activity.RESULT_OK){
            ...
            if(holdingActivity != null)
            {
                ...
                mDiplayedTask.setPicPath ( mCurrentPhotoPath );
                TaskListContent.Task task = TaskListContent.ITEM_MAP.get ( mDiplayedTask.id );
                if(task != null)
                {
